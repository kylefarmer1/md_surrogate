...
class LearnedSimulator(snt.Module):  # noqa
    ...
    def _encoder_preprocessor(self, graph, next_position, s):
        ...
        if self._edge_types == 'distances':
            edge_features = tf.concat([edges, norm], axis=1)
        elif self._edge_types == 'displacements':
            edge_features = tf.concat([edges, displacements], axis=1)
        else:
            edge_features = tf.concat([edges, displacements / tf.math.reduce_max(displacements),
                                       norm], axis=1)
        ...
        return gn.graphs.GraphsTuple(
            ...
            globals=tf.cast(graph.globals, dtype=tf.float64),
            n_node=graph.n_node,
            n_edge=graph.n_edge,
    ...
    def get_step_accelerations_and_energies(self, graph):
        next_position = graph.nodes[:, :2]
        s = tf.gather(next_position, graph.senders)
        with tf.GradientTape() as tape:
            tape.watch(s)
        ...
        force_per_edge = tape.gradient(tot_energies, s) * -1
        return energies, force_per_edge
