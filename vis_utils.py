...
def animate_trajectory_and_energies(rollout, system_size, energies1, title=None, energies2=None,
                                    outpath='movie', fps=10):
    ...
    hist_length = 8
    ax[0].set_xlim(xlo-.1*lx, xhi+.1*lx)
    ax[0].set_ylim(ylo-.1*ly, yhi+.1*ly)
    ax[0].plot([xlo, xhi], [yhi, yhi], 'k-', lw=4)
    ax[0].plot([xlo, xhi], [ylo, ylo], 'k-', lw=4)
    ax[0].plot([xlo, xlo], [ylo, yhi], 'k-', lw=4)
    ax[0].plot([xhi, xhi], [ylo, yhi], 'k-', lw=4)
    lines, hists = [], []
    ...
    # energies
    energy_colors = ['#003f5c', '#bc5090', '#ffa600']
    steps = np.arange(0, energies1.shape[0])
    ax[1].plot(steps, energies1[:, 0], c=energy_colors[0], label='PE')
    ax[1].plot(steps, energies1[:, 1], c=energy_colors[1], label='KE')
    ax[1].plot(steps, energies1[:, 0] + energies1[:, 1], c=energy_colors[2], label='Etot')
    b_line, = ax[1].plot([steps[0], steps[0]], list(ax[1].get_ylim()), c='k')

    if energies2 is not None:
        ax[1].plot(steps, energies2[:, 0], linestyle='dashed', c=energy_colors[0], )
        ax[1].plot(steps, energies2[:, 1], linestyle='dashed', c=energy_colors[1], )
        ax[1].plot(steps, energies2[:, 0] + energies2[:, 1], linestyle='dashed', c=energy_colors[2], )

    ax[1].set_xlabel('step', fontsize=20)
    ax[1].set_ylabel('energy', fontsize=20)
    ...
    for atom_ind in range(num_atoms):
        ...
        hist, = ax[0].plot(prev_seq[:, 0], prev_seq[:, 1], lw=32, color=colors[atom_ind], alpha=0.5)
        hist.set_solid_capstyle('round')

        line, = ax[0].plot([rollout[0, atom_ind, 0]], [rollout[0, atom_ind, 1]],
    ...
    def animate(i):
        ...
        prev_seq_mask[prev_seq_mask < 0] = 0
        b_line.set_data([steps[i], steps[i]], list(ax[1].get_ylim()))
...
def animate_trajectory(rollout, system_size, title=None, outpath='movie', fps=10):
    """Animate two trajectory of a rollout
...
def animate_two_trajectory(rollout1, rollout2, system_size, titles=None, outpath='movie', gif=False, fps=5, hist_length=8):
...
def animate_two_trajectory_and_energies(rollout1, rollout2, system_size, energies1, energies2,
                                      titles=None, outpath='movie', gif=False, rev=False, fps=10):
    """animate two trajectories side by side
    ...
    :return:
    """
    fig, ax = plt.subplot_mosaic([['left', 'right'], ['bottom', 'bottom']], constrained_layout=True, figsize=(16, 16))

    # energies
    energy_colors = ['#003f5c', '#bc5090', '#ffa600']
    steps = np.arange(0, energies1.shape[0])
    ax['bottom'].plot(steps, energies1[:, 0], c=energy_colors[0], label='PE')
    ax['bottom'].plot(steps, energies1[:, 1], c=energy_colors[1], label='KE')
    ax['bottom'].plot(steps, energies1[:, 0] + energies1[:, 1], c=energy_colors[2], label='Etot')
    b_line, = ax['bottom'].plot([steps[0], steps[0]], list(ax['bottom'].get_ylim()), c='k')

    if energies2 is not None:
        ax['bottom'].plot(steps, energies2[:, 0], linestyle='dashed', c=energy_colors[0], )
        ax['bottom'].plot(steps, energies2[:, 1], linestyle='dashed', c=energy_colors[1], )
        ax['bottom'].plot(steps, energies2[:, 0] + energies2[:, 1], linestyle='dashed', c=energy_colors[2], )

    ax['bottom'].set_xlabel('step', fontsize=20)
    ax['bottom'].set_ylabel('energy', fontsize=20)
    ax['bottom'].tick_params(axis='both', which='major', labelsize=15, direction='in')
    ax['bottom'].legend()
    ...
    for pos in ['left', 'right']:
        ax[pos].set_xlim(xlo-.1*lx, xhi+.1*lx)
        ax[pos].set_ylim(ylo-.1*ly, yhi+.1*ly)

        ax[pos].plot([xlo, xhi], [yhi, yhi], 'k-', lw=4)
        ax[pos].plot([xlo, xhi], [ylo, ylo], 'k-', lw=4)
        ax[pos].plot([xlo, xlo], [ylo, yhi], 'k-', lw=4)
        ax[pos].plot([xhi, xhi], [ylo, yhi], 'k-', lw=4)
    ...
    for atom_ind in range(num_atoms):
        prev_seq = rollout1[prev_seq_mask, atom_ind, :2]
        hist1, = ax['left'].plot(prev_seq[:, 0], prev_seq[:, 1], lw=30, color=colors[atom_ind], alpha=0.5)
        hist1.set_solid_capstyle('round')

        prev_seq = rollout2[prev_seq_mask, atom_ind, :2]
        hist2, = ax['right'].plot(prev_seq[:, 0], prev_seq[:, 1], lw=30, color=colors[atom_ind], alpha=0.5)
        hist2.set_solid_capstyle('round')

        if rev:
            line1b, = ax['left'].plot([rollout1[0, atom_ind, 0]], [rollout1[0, atom_ind, 1]],
                                     'o', markersize=32, markeredgewidth=4, markeredgecolor='k', color='none')
            line2b, = ax['right'].plot([rollout2[0, atom_ind, 0]], [rollout2[0, atom_ind, 1]],
                                      'o', markersize=32, markeredgewidth=4, markeredgecolor='k', color='none')

        line1, = ax['left'].plot([rollout1[0, atom_ind, 0]], [rollout1[0, atom_ind, 1]],
                                 'o', markersize=29, markeredgewidth=4, markeredgecolor='none', color=colors[atom_ind])
        line2, = ax['right'].plot([rollout2[0, atom_ind, 0]], [rollout2[0, atom_ind, 1]],
                                  'o', markersize=29, markeredgewidth=4, markeredgecolor='none', color=colors[atom_ind])
    ...
    def animate(i):
        prev_seq_mask = np.arange(i - hist_length, i + 1, dtype=np.int32)
        prev_seq_mask[prev_seq_mask < 0] = 0
        b_line.set_data([steps[i], steps[i]], list(ax['bottom'].get_ylim()))
        for idx, line in enumerate(lines1):
            prev_seq1 = rollout1[prev_seq_mask, idx, :2]
            prev_seq2 = rollout2[prev_seq_mask, idx, :2]
            ...
            title1 = f'{titles[0]} - step:{i}' if titles else f'step: {i}'
            title2 = f'{titles[1]} - step:{i}' if titles else f'step: {i}'
            ax['left'].set_title(title1)
            ax['right'].set_title(title2)
...
def animate_trajectory_with_acc(rollout1, rollout2, system_size, outpath='movie', fps=5):
    ...
    for atom_ind in range(num_atoms):
        ...
        node, = ax.plot(
            ...
            'o', markersize=12, markeredgewidth=4.0, markeredgecolor='k', color=colors[atom_ind])
        nodes.append(node)
...
def plot_losses(loss1, loss2, ax, inc=1):
    ...
    num_steps = loss1.shape[0]*inc
    colors = ['#003f5c', '#ffa600']

    line, = ax.plot(np.arange(0, num_steps, inc), loss1, c=colors[0], label='rollout')
    line2, = ax2.plot(np.arange(0, num_steps, inc), loss2, c=colors[1], label='one-step')
